...
PR Creation (when --pr is enabled):
  ...
  2. Body includes summary, PLAN.md link, execution item counts
...
/**
 * Get execution item completion summary
 */
...
/**
 * Format PR body with summary, PLAN.md link, and execution counts
 */
function formatPRBody(
  ...
  const itemSummary = getItemSummary(repoRoot, streamId)
  ...
  const lines: string[] = [
    ...
    `| Status | Count |`,
    `|--------|-------|`,
    `| Completed | ${itemSummary.completed} |`,
    `| In Progress | ${itemSummary.inProgress} |`,
    `| Pending | ${itemSummary.pending} |`,
    `| Blocked | ${itemSummary.blocked} |`,
    `| Cancelled | ${itemSummary.cancelled} |`,
...
async function validateCompletion(
  ...
  // Get execution item summary for warnings
  const itemSummary = getItemSummary(repoRoot, stream.id)
  if (itemSummary.inProgress > 0) {
    warnings.push(`${itemSummary.inProgress} item(s) still in progress`)
  }
  if (itemSummary.pending > 0) {
    warnings.push(`${itemSummary.pending} item(s) still pending`)
  }
  if (itemSummary.blocked > 0) {
    warnings.push(`${itemSummary.blocked} item(s) blocked`)
  }
