...
4. Treat each dispatched batch as blocking: wait for all its workers to return before diagnosing outcomes, requesting plan changes, or dispatching more work. Address returned blockers and gaps immediately using the Review and Fixes procedure, even before a checkpoint is reached.
...
2. Before coordinating a retry or fix attempt, read `resources/RETRY_POLICY.md` relative to this skill. Use the user's specified limits; while the numeric defaults remain a draft, ask for limits when none were supplied.
