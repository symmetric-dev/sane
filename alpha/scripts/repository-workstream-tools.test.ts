...
describe("repository-aware Alpha workstream tools", () => {
  ...
  beforeEach(async () => {
    ...
    for (const source of [
       ...
       "design/section/SPEC.md", "execution/JOB.md", "implementation/REPORT.md",
    ]) {
  ...
  async function bootstrap(relativePath: string): Promise<string> {
  ...
  test("dry-run creation validates but creates and selects nothing", async () => {
    await createSaneRepositoryWorkstream({
      implementationRepository, workstreamPath: "02-dry-run", templateRoot, dryRun: true, write: () => {},
    })
  ...
  test("rejects traversal, paths redirected outside the repository, and unbootstrapped selections", async () => {
    ...
    await expect(createSaneRepositoryWorkstream({ implementationRepository, workstreamPath: "redirect/new", templateRoot, write: () => {} })).rejects.toThrow("outside")
  ...
  test("provisions approved roles from current selection or a validated override", async () => {
    const first = await bootstrap("01-first")
    const second = await bootstrap("02-second")
    await provisionSaneRole({ implementationRepository, role: "research", templateRoot, write: () => {} })
    expect(await readFile(join(second, "research", "INDEX.md"), "utf8")).toBe("research/INDEX.md\n")
    await provisionSaneRole({ implementationRepository, role: "design", workstreamPath: "01-first", templateRoot, write: () => {} })
    expect(await readFile(join(first, "design", "SPEC.md"), "utf8")).toBe("design/SPEC.md\n")
  })
  ...
  test("enforces role and stage constraints and refuses existing artifacts", async () => {
    ...
    expect(await readFile(join(workstream, "design", "stages", "01-foundation", "SPEC.md"), "utf8")).toBe("design/stage/SPEC.md\n")
  ...
  test("validates every role source before creating any destination", async () => {
    ...
    await rm(join(templateRoot, "research", "TECH_BRIEF.md"))
